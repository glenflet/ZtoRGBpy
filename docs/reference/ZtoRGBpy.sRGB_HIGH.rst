...
.. py:data:: sRGB_HIGH
    :annotation: = ZtoRGBpy.RGBColorProfile((2126.0, 7152.0, 722.0), 0.5)

    High Contrast sRGB Color Profile.

    Defines a high contrast color profile using gamma = :math:`\frac{1}{2}`, and color weights
    of (2126.0, 7152.0, 772.0), matching the sRGB color space defined by IEC :cite:`sRGB_HIGH-IECsRGB`

    .. seealso:: :py:const:`sRGB_LOW`, :py:const:`sRGB`
    ...
    .. plot::
        ...
        >>> import ZtoRGBpy
        >>> ZtoRGBpy.colorwheel(profile=ZtoRGBpy.sRGB_HIGH,grid=True)
        >>> plt.show()
